...
from lib.html5lib.constants import voidElements, booleanAttributes, spaceCharacters
from lib.html5lib.constants import rcdataElements, entities, xmlEntities
from lib.html5lib import utils
...
class HTMLSerializer(object):
    ...
    def serialize(self, treewalker, encoding=None):
        ...
        if encoding and self.inject_meta_charset:
            from lib.html5lib.filters.inject_meta_charset import Filter
            treewalker = Filter(treewalker, encoding)
        # XXX: WhitespaceFilter should be used before OptionalTagFilter
        # for maximum efficiently of this latter filter
        if self.strip_whitespace:
            from lib.html5lib.filters.whitespace import Filter
            treewalker = Filter(treewalker)
        if self.sanitize:
            from lib.html5lib.filters.sanitizer import Filter
            treewalker = Filter(treewalker)
        if self.omit_optional_tags:
            from lib.html5lib.filters.optionaltags import Filter
            treewalker = Filter(treewalker)
